...
public class PolicyRulesUtil {
    ...
    public static List<PolicyRules> resolve(PolicyRules policyRules, VariablePinner variables)
        ...
        if (Variable.isVariable(policyRules.getId())) {
            ...
            try {
                ...
                try {
                    Boolean valid = applyConditions(new JSONObject(conditions), variables);
     ...
     */
    static private Boolean applyConditions(JSONObject conditionObject, VariablePinner variables)
        throws RuntimeException {
        Boolean valid;
        try {
            for (String cond : conditionObject.keySet()) {
                valid = ConditionUtil.apply(new JSONObject(cond), variables);
